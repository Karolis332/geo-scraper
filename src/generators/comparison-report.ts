...
const STRINGS_EN: ComparisonStrings = {
  pageTitle: 'GEO Comparison Report',
  exportPdf: 'Download PDF',
  exportCsv: 'Export CSV',
  exportHint: 'Use browser "Save as PDF" in the print dialog',
  complianceTitle: 'GEO Compliance',
  current: 'Current',
  afterDeploy: 'After Deploy',
  summaryImproves: 'By deploying the generated files, your GEO score improves',
  summaryChanges: 'By deploying the generated files, your GEO score changes',
  summaryNoChange: 'By deploying the generated files, your GEO score stays',
  geoServiceScore: 'GEO Service Score',
  overallScoreWithSeo: 'Overall Score (GEO + SEO)',
  summaryFixing: 'fixing',
  summaryAutoImprovements: 'Automatic improvements are included.',
  summaryNeedsAttention: 'still need your attention',
  weFix: 'We Fix These',
  weFixDesc: 'Deploy the generated files and these items are solved instantly.',
  needsAttention: 'Needs Your Attention',
  needsAttentionDesc: 'These require content or configuration changes on your website.',
  alreadyGood: 'Already Good',
  generatedBy: 'Generated by <strong>geo-scraper</strong> &mdash; Generative Engine Optimization',
  now: 'Now',
  after: 'After',
  item: 'Item',
  category: 'Category',
  ...
  scoreUnchanged: 'No change',
};

const STRINGS_LT: ComparisonStrings = {
  pageTitle: 'GEO palyginimo ataskaita',
  exportPdf: 'Atsisiųsti PDF',
  exportCsv: 'Eksportuoti CSV',
  exportHint: 'Naudokite naršyklės „Išsaugoti kaip PDF" spausdinimo lange',
  complianceTitle: 'GEO atitiktis',
  current: 'Dabar',
  afterDeploy: 'Po diegimo',
  summaryImproves: 'Įdiegus sugeneruotus failus, jūsų GEO balas pagerėja',
  summaryChanges: 'Įdiegus sugeneruotus failus, jūsų GEO balas pasikeičia',
  summaryNoChange: 'Įdiegus sugeneruotus failus, jūsų GEO balas nesikeičia',
  geoServiceScore: 'GEO paslaugos balas',
  overallScoreWithSeo: 'Bendras balas (GEO + SEO)',
  summaryFixing: 'Automatiškai sutvarkoma',
  summaryAutoImprovements: 'Automatiniai patobulinimai jau įtraukti.',
  summaryNeedsAttention: 'vis dar reikalauja jūsų dėmesio',
  weFix: 'Sutvarkome automatiškai',
  weFixDesc: 'Įdiegus sugeneruotus failus, šie punktai išsprendžiami iš karto.',
  needsAttention: 'Reikia jūsų dėmesio',
  needsAttentionDesc: 'Šiems punktams reikia turinio arba konfigūracijos pakeitimų jūsų svetainėje.',
  alreadyGood: 'Jau tvarkoje',
  generatedBy: 'Sugeneravo <strong>geo-scraper</strong> &mdash; Generatyvinio variklio optimizavimas',
  now: 'Dabar',
  after: 'Po',
  item: 'Elementas',
  category: 'Kategorija',
  beforeScore: 'Balas prieš',
  afterScore: 'Balas po',
  delta: 'Pokytis',
  status: 'Būsena',
  statusImproved: 'pagerėjo',
  statusUnchanged: 'nepakito',
  scoreUnchanged: 'Be pokyčio',
};
...
function priorityLabel(cat: string, lang: 'en' | 'lt'): string {
  if (lang === 'lt') {
    if (cat === 'ai_infrastructure') return 'DI infra';
    if (cat === 'content_quality') return 'Turinys';
    if (cat === 'ai_discoverability') return 'Aptikimas';
...
function detectComparisonLanguage(crawlResult: SiteCrawlResult): 'en' | 'lt' {
  const ltByMeta = crawlResult.pages.some((page) => page.meta.language?.toLowerCase().startsWith('lt'));
  if (ltByMeta) return 'lt';
  if (crawlResult.domain.toLowerCase().endsWith('.lt')) return 'lt';
  return 'en';
}

function itemLabel(count: number, lang: 'en' | 'lt'): string {
  if (lang === 'lt') return count === 1 ? 'punktas' : 'punktų';
...
export function generateComparisonHtml(
  ...
  const lang = detectComparisonLanguage(crawlResult);
  const s = lang === 'lt' ? STRINGS_LT : STRINGS_EN;
  ...
  const beforeGeoScore = calculateGeoServiceScore(before.items);
  const afterGeoScore = calculateGeoServiceScore(after.items);
  const geoDelta = afterGeoScore - beforeGeoScore;
  const scoreDelta = after.overallScore - before.overallScore;
  const scoreDeltaLabel = scoreDelta > 0 ? `+${scoreDelta}` : `${scoreDelta}`;
  const scoreDeltaClass = scoreDelta > 0 ? 'delta-positive' : scoreDelta < 0 ? 'delta-negative' : 'delta-neutral';
  ...
  // Summary sentence
  const summaryLead = scoreDelta > 0
    ? s.summaryImproves
    : scoreDelta < 0
      ? s.summaryChanges
      : s.summaryNoChange;
  const summaryMiddle = scoreDelta === 0
    ? ` <strong style="color:${beforeColor}">${before.overallScore}</strong>.`
    : lang === 'lt'
      ? ` nuo <strong style="color:${beforeColor}">${before.overallScore}</strong> iki <strong style="color:${afterColor}">${after.overallScore}</strong>.`
      : ` from <strong style="color:${beforeColor}">${before.overallScore}</strong> to <strong style="color:${afterColor}">${after.overallScore}</strong>.`;
  const summaryTail = scoreDelta === 0
    ? ` ${s.scoreUnchanged}.`
    : weFixItems.length > 1
      ? ` ${s.summaryFixing} <strong>${weFixItems.length} ${itemLabel(weFixItems.length, lang)}</strong>.`
      : ` ${s.summaryAutoImprovements}`;
  const summaryAttention = clientFixItems.length > 0
    ? ` ${clientFixItems.length} ${itemLabel(clientFixItems.length, lang)} ${s.summaryNeedsAttention}.`
    : '';
  const geoSummary = lang === 'lt'
    ? ` ${s.geoServiceScore}: <strong>${beforeGeoScore}</strong> → <strong>${afterGeoScore}</strong> (${geoDelta > 0 ? '+' : ''}${geoDelta}).`
    : ` ${s.geoServiceScore}: <strong>${beforeGeoScore}</strong> → <strong>${afterGeoScore}</strong> (${geoDelta > 0 ? '+' : ''}${geoDelta}).`;
  const summaryText = summaryLead + summaryMiddle + summaryTail + summaryAttention + geoSummary;
    ...
    return `
      <div class="item-card${showImprovement ? ' improved' : ''}">
        <div class="item-header">
          ...
          <div class="item-meta">
            <span class="priority priority-${b.category}">${priorityLabel(b.category, lang)}</span>
            ${delta !== 0 ? `<span class="delta-pill${delta < 0 ? ' delta-pill-negative' : ''}">${delta > 0 ? '+' : ''}${delta}</span>` : ''}
          </div>
        </div>
        <div class="bars">
          <div class="bar-row">
            <span class="bar-label">${s.now}</span>
            <div class="bar-track">
          ...
          <div class="bar-row">
            <span class="bar-label">${s.after}</span>
            <div class="bar-track">
...
<head>
  ...
  <title>${s.pageTitle} — ${siteName}</title>
  <style>
    :root {
      --bg: #09090b;
      --surface: #131316;
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .score-meta-pill strong { color: var(--text); }
    ...
    @media (max-width: 640px) {
      ...
      .score-meta-row { flex-direction: column; align-items: center; }
    }
    @media print {
...
<body>
  <div class="export-toolbar">
    <button onclick="window.print()">${s.exportPdf}</button>
  ...
  <!-- Hero -->
  <div class="hero">
    <h1>${s.complianceTitle}</h1>
    <div class="site-name">${siteName}</div>
    <div class="site-url"><a href="${baseUrl}">${baseUrl}</a></div>

    <div class="score-row">
      <div class="score-block">
        <div class="circle-label">${s.current}</div>
      ...
      <div class="arrow-block">
        ...
        <span class="delta-pill-hero ${scoreDeltaClass}">${scoreDeltaLabel}</span>
      </div>

      <div class="score-block">
        <div class="circle-label">${s.afterDeploy}</div>
    ...
    <div class="score-meta-row">
      <span class="score-meta-pill">${s.geoServiceScore}: <strong>${beforeGeoScore}</strong> → <strong>${afterGeoScore}</strong> (${geoDelta > 0 ? '+' : ''}${geoDelta})</span>
      <span class="score-meta-pill">${s.overallScoreWithSeo}: <strong>${before.overallScore}</strong> → <strong>${after.overallScore}</strong> (${scoreDelta > 0 ? '+' : ''}${scoreDelta})</span>
    </div>
  </div>
  ...
  <div class="section">
    <div class="section-header">
      ...
      <div class="section-title">${s.weFix}</div>
      <span class="section-count">${weFixItems.length} ${itemLabel(weFixItems.length, lang)}</span>
    </div>
    <p style="color:var(--text-dim);font-size:0.9rem;margin:-0.75rem 0 1.25rem 0;">${s.weFixDesc}</p>
${weFixHtml}
  </div>` : ''}
  ...
  <div class="section">
    <div class="section-header">
      ...
      <div class="section-title">${s.needsAttention}</div>
      <span class="section-count">${clientFixItems.length} ${itemLabel(clientFixItems.length, lang)}</span>
    </div>
    <p style="color:var(--text-dim);font-size:0.9rem;margin:-0.75rem 0 1.25rem 0;">${s.needsAttentionDesc}</p>
${clientFixHtml}
  </div>` : ''}
  ...
  <div class="section">
    <div class="section-header">
      ...
      <div class="section-title">${s.alreadyGood}</div>
      <span class="section-count">${alreadyGoodItems.length} ${itemLabel(alreadyGoodItems.length, lang)}</span>
    </div>
${alreadyGoodHtml}
  </div>` : ''}
...
<script>
function exportCsv() {
  const rows = [['${s.item}','${s.category}','${s.beforeScore}','${s.afterScore}','${s.delta}','${s.status}']];
  document.querySelectorAll('.item-card').forEach(el => {
    ...
    const status = el.classList.contains('improved') ? '${s.statusImproved}' : '${s.statusUnchanged}';
